...
const linaAlerts = [
  {
    ...
    text: "Denizli Residence şantiyesinde beton stoğu kritik seviyeye düştü.",
    tone: "orange",
  },
  {
    icon: <AlertTriangle size={18} />,
    text: "Yılmaz İnşaat hakedişi 15 gündür onay bekliyor.",
    tone: "orange",
  },
  {
    icon: <CalendarClock size={18} />,
    text: "3 taşeronun sözleşme süresi önümüzdeki hafta doluyor.",
    tone: "blue",
  },
  {
    icon: <CheckCircle2 size={18} />,
    text: "Merkez Lofts projesinde satış oranı hedefin altında.",
    tone: "green",
  },
];
      ...
      <section className="mx-auto flex w-full max-w-7xl flex-col gap-4">
        <header className="rounded-3xl border border-[#C7D6E8] bg-white px-4 py-4 shadow-sm">
          <div className="text-center">
            ...
            <h1 className="mt-1 text-2xl font-extrabold tracking-tight text-[#1F2937]">
              İnşaat Firması CRM
            </h1>
        ...
        <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          {stats.map((item) => (
            <article
              key={item.title}
              className="rounded-3xl border border-[#C7D6E8] bg-white p-4 shadow-sm"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="break-words text-sm font-bold text-[#2563EB]">
                    {item.title}
                  </p>
                ...
                <div
                  className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border ${toneClasses[item.tone]}`}
                >
                  {item.icon}
                </div>
              </div>
        ...
        <section className="rounded-3xl border border-[#C7D6E8] bg-white p-4 shadow-sm">
          <h2 className="text-center text-base font-extrabold text-[#1F2937]">
            Hızlı İşlemler
          </h2>
          ...
          <div className="mt-4 grid grid-cols-4 gap-2 sm:grid-cols-4 lg:grid-cols-8">
            {quickActions.map((item) => (
              ...
              >
                ...
                <span
                  className={`mt-2 flex h-11 w-11 items-center justify-center rounded-2xl border ${toneClasses[item.tone as keyof typeof toneClasses]}`}
                >
                  {item.icon}
                </span>
        ...
        <section className="grid gap-4 lg:grid-cols-[1.45fr_1fr]">
          <article className="rounded-3xl border border-[#C7D6E8] bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              {sites.map((site) => (
                <div
                  key={site.name}
                  className="rounded-2xl border border-[#C7D6E8] bg-[#F8FAFC] p-3"
                >
                  ...
                  <h3 className="mt-3 text-center text-sm font-extrabold text-[#1F2937]">
                    {site.name}
                  </h3>
          ...
          <article className="rounded-3xl border border-[#C7D6E8] bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              ...
              <button type="button" className="text-xs font-bold text-[#2563EB]">
                Tümünü Gör
              </button>
            </div>
            ...
            <div className="mt-4 space-y-3">
              {linaAlerts.map((alert, index) => (
                <div
                  key={index}
                  className="flex gap-3 rounded-2xl border border-[#C7D6E8] bg-[#F8FAFC] p-3"
                >
                  <span
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          ...
          <SmallListCard
            ...
            rows={materialRequests.map(([name, amount]) => ({
              left: name,
              right: amount,
              badge: "Açık Talep",
            }))}
          ...
          <SmallListCard
            ...
            icon={<Users size={18} />}
            rows={salesTeam.map(([name, detail, amount]) => ({
              left: name,
              right: amount,
              badge: detail,
            }))}
      ...
      <div className="mt-4 space-y-3">
        {rows.map((row) => (
          <div
            ...
            className="grid grid-cols-[1fr_auto] gap-2 rounded-2xl bg-[#F8FAFC] p-3"
          >
            <div className="min-w-0">
              <p className="break-words text-sm font-extrabold text-[#1F2937]">
                {row.left}
              </p>
            ...
            <p className="text-right text-sm font-extrabold text-[#1F2937]">
              {row.right}
            </p>
          </div>
        ))}
