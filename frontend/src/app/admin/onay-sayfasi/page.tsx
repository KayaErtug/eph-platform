...
export default function AdminOnaySayfasiPage() {
  ...
  const normalizedUserRole = String(user?.role || "").toLocaleUpperCase("tr-TR");
  const isModerator = normalizedUserRole === "MODERATOR";
  const isAdmin = normalizedUserRole === "ADMIN";
  const isSoftwareTeam = normalizedUserRole === "SUPER_ADMIN";
  const canMakeFinalDecision = isAdmin || isSoftwareTeam;
  const canSendToSoftwareTeam = isModerator || isAdmin;
  ...
  useEffect(() => {
    ...
    if (!["MODERATOR", "ADMIN", "SUPER_ADMIN"].includes(normalizedUserRole)) {
      router.replace("/dashboard");
      return;
    }
  }, [normalizedUserRole, router, user]);
    ...
    <main className="applications-page">
      ...
      <section className="page-shell">
        <header className="page-head">
          <a href={isModerator || isAdmin ? "/dashboard" : "/admin"} className="back-button" aria-label="Geri dön">
            <ArrowLeft size={22} />
          </a>

          <div>
            <h1>Onay Sayfası</h1>
            <p>Başvuru inceleme, not ve Yazılım Ekibi karar merkezi</p>
          </div>
        ...
        <section className="approval-tabs" aria-label="Onay sayfası menüsü">
          <button type="button" className="approval-tab active">📥 Katılım Talepleri</button>
          <button type="button" className="approval-tab">👤 Referans Talepleri</button>
          <button type="button" className="approval-tab">🛡️ Moderatör Talepleri</button>
          <button type="button" className="approval-tab software">💻 Yazılım Ekibi</button>
        </section>
                  ...
                  <div className="decision-line">
                    ...
                    <div className="card-actions">
                      ...
                      <button
                        type="button"
                        className="approve"
                        disabled={!canMakeFinalDecision || busyId === item.id || item.status !== "PENDING"}
        ...
        <div className="modal-backdrop" role="dialog" aria-modal="true">
          <section className="detail-modal">
            ...
            <div className="modal-actions">
              ...
              {canSendToSoftwareTeam ? (
                <button className="secondary-button software-team-button" onClick={handleSaveNote} disabled={busyId === selected.id}>
                  Yazılım Ekibine Gönder
                </button>
              ) : null}
              <button
                className="danger-button"
                onClick={() => handleStatusChange(selected.id, "REJECTED")}
                disabled={!canMakeFinalDecision || busyId === selected.id || selected.status !== "PENDING"}
              >
                Reddet
              </button>
              <button
