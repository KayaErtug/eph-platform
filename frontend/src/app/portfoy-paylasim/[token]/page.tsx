...
function getSpecs(unit: SharedUnit): SpecItem[] {
  const specs: SpecItem[] = [
    ...
    {
      label: "İşlem",
      value: unit.status ? String(unit.status).replaceAll("_", " ") : "Portföy",
    },
  ];
...
export default function PortfolioSharePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await api.get(`/portfolio-share/${token}`);
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#F5F3FF]">
        <p className="text-[13px] font-black text-[#6D5FA6]">Yükleniyor...</p>
      </div>
    );
  }
  ...
  if (error || !unit) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-3 bg-[#F5F3FF] px-6 text-center">
        <Building2 size={40} className="text-[#A78BDA]" />
        <p className="max-w-xs text-[13px] font-black leading-5 text-[#2E1065]">
  ...
  return (
    <div className="min-h-screen bg-[#F5F3FF] px-3 py-6">
      <div className="mx-auto w-full max-w-[460px]">
        <p className="mb-4 text-center text-[11px] font-black uppercase tracking-[0.14em] text-[#7C3AED]">
          EPH · Portföy Paylaşımı
        </p>

        <section className="overflow-hidden rounded-[26px] border-2 border-[#DDD6FE] bg-white shadow-[0_18px_44px_rgba(46,16,101,0.12)]">
          <div className="relative h-[280px] bg-[#F3EEFF] sm:h-[320px]">
            ...
            {galleryImages.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-3 top-1/2 z-[2] flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border-2 border-white/80 bg-white/92 text-[#2E1065] shadow-[0_10px_22px_rgba(46,16,101,0.20)]"
                >
                  <ChevronLeft size={20} />
                </button>
                <button
                  ...
                  className="absolute right-3 top-1/2 z-[2] flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border-2 border-white/80 bg-white/92 text-[#2E1065] shadow-[0_10px_22px_rgba(46,16,101,0.20)]"
                >
          ...
          {galleryImages.length > 1 && (
            <div className="flex gap-1.5 overflow-x-auto border-t-2 border-[#EDE7FB] bg-[#FAF8FF] p-2 [-webkit-overflow-scrolling:touch]">
          ...
          <div className="px-4 py-4 text-center">
            <p className="text-[10px] font-black uppercase tracking-[0.12em] text-[#7C3AED]">
              {typeLabel(unit.type)} • {unit.ephId}
            </p>
            <h1 className="mt-1 text-[19px] font-black leading-tight text-[#2E1065]">
              {unit.project?.name || "EPH Portföyü"}
            </h1>
            <p className="mt-1 flex items-center justify-center gap-1 text-[12px] font-bold text-[#6D5FA6]">
              <MapPin size={13} /> {location}
            </p>
            <p className="mt-2 inline-flex items-center justify-center rounded-full bg-[#7C3AED] px-5 py-1.5 text-[17px] font-black text-white">
              {formatPrice(unit.price, unit.priceCurrency)}
            </p>
          </div>

          <div className="flex flex-wrap justify-center gap-1.5 border-t-2 border-[#EDE7FB] bg-white px-3 py-2.5">
          ...
          <div className="grid grid-cols-2 gap-1.5 border-t-2 border-[#EDE7FB] bg-[#FAF8FF] p-2.5">
            {specs.map((spec) => (
              <div
                key={spec.label}
                className="rounded-[14px] bg-white px-2 py-2 text-center"
              >
                <p className="text-[8.5px] font-black uppercase tracking-[0.05em] text-[#6D5FA6]">
                  {spec.label}
                </p>
          ...
          {featureLabels.length > 0 && (
            <div className="flex flex-wrap justify-center gap-1.5 border-t-2 border-[#EDE7FB] px-4 py-3">
              {featureLabels.map((feature) => (
                <span
                  key={feature}
                  className="rounded-full bg-[#F3EEFF] px-2.5 py-1 text-[10px] font-black text-[#6D28D9]"
                >
                  {feature}
                </span>
              ))}
          ...
          {unit.description && (
            <div className="border-t-2 border-[#EDE7FB] px-4 py-3 text-center text-[12.5px] font-bold leading-5 text-[#5B4B82]">
              {unit.description}
            </div>
          )}
        </section>
        ...
        {unit.sharedBy && (
          <section className="mt-3 overflow-hidden rounded-[22px] border-2 border-[#DDD6FE] bg-white p-4 text-center shadow-[0_10px_22px_rgba(46,16,101,0.06)]">
            <p className="text-[10px] font-black uppercase tracking-[0.12em] text-[#7C3AED]">
              Bu İlanı Sizinle Paylaşan
            </p>
            <p className="mt-1 text-[15px] font-black text-[#2E1065]">
        ...
        <p className="mx-auto mt-4 max-w-[340px] text-center text-[10px] font-bold leading-4 text-[#A78BDA]">
